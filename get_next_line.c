...
/*   Updated: 2023/01/27 07:47:09 by tayou            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_newline_point(char *backup)
{
	int	i;
...
char	*ft_substr(char *backup, int start, int newline_point)
{
	char	*substring;
	int		size;
	int		i;
...
char	*get_line(char *line, char *backup)
{
	...
	if (ft_strchr(backup, '\n') != NULL)
	{
		newline_point = get_newline_point(backup);
		line = ft_substr(backup, 0, newline_point);
		temp = backup;
		backup_size = ft_strlen(backup);
		i = newline_point + 1;
		backup = ft_substr(temp, i, backup_size - i);
